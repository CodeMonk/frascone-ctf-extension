...
preview:function(command, input, preview, terminal) {
      ...
      terminal.previewAjaxGet(preview, url, function(data){
        ...
        var time = (jQuery("modified",data).get(0).textContent.replace(timeFormat,"$2 $3,$1 $4:$5:$6"));
        preview.previewTemplateFile('ge.html',
          { messages:messages, time:(new Date(time)).toLocaleString(), fullcount:fullcount }
        );
      });
